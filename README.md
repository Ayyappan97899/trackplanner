# TrackPlanner

TrackPlanner is a simple task management and calendar app built with React and Chakra UI. It helps you organize, search, and filter tasks by status or week, all in a clean month-view interface.

## Features

- Create, edit, and view tasks.
- Month-view calendar for tasks.
- Search tasks by title.
- Filter tasks by status or week.

## Tech Stack

- React.js, Chakra UI, Zustand, date-fns, react-icons

## Setup

```bash
git clone https://github.com/Ayyappan97899/trackplanner.git
cd trackplanner
npm install
npm run dev
```
